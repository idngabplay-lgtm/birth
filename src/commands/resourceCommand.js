...
// Store pending assignments (temporary)
...
class ResourceCommand {
  ...
  async handleAssignPIC(sock, sender, messageText, isAdmin) {
    ...
    try {
      ...
      if (result.matches.length > 1) {
        ...
        message += `Contoh: /confirmAssign [Folder ID] | [Angka]`;
      ...
      // Exact match found or single match
  ...
  // Confirm assignment
  async handleConfirmAssign(sock, sender, messageText, isAdmin) {
    if (!isAdmin) {
      await sock.sendMessage(sender, {
        text: "❌ Akses ditolak! Hanya admin yang bisa assign PIC.",
      });
    ...
    if (answer.toUpperCase() !== "YA") {
      ...
      await sock.sendMessage(sender, {
        text: "❌ Assign PIC dibatalkan.",
      });
      return;
    }

    try {
      // Get WhatsApp number if available from people list or ask
      const result = await resourceService.assignPIC(
        parseInt(folderId),
        pending.selectedName,
        null,
      );
  ...
  // ===== DELETE ASSIGNMENT =====
  async handleDeleteAssign(sock, sender, messageText, isAdmin) {
    if (!isAdmin) {
      await sock.sendMessage(sender, {
        text: "❌ Akses ditolak! Hanya admin yang bisa menghapus assign.",
      });
      return;
    }
    ...
    try {
      const result = await resourceService.deleteAssignment(
        parseInt(folderId),
        password,
      );
  ...
  // ===== SELECT NAME FROM MULTIPLE MATCHES =====
  async handleSelectName(sock, sender, messageText, isAdmin) {
    if (!isAdmin) {
      await sock.sendMessage(sender, {
        text: "❌ Akses ditolak!",
      });
    ...
    if (
      !pending ||
      pending.folderId !== parseInt(folderId) ||
      pending.step !== "select_name"
    ) {
      await sock.sendMessage(sender, {
  ...
  // Show folder details (existing method)
  ...
  // ===== CHANGE PASSWORD =====
  async handleChangePassword(sock, sender, messageText, isAdmin) {
    if (!isAdmin) {
      await sock.sendMessage(sender, {
        text: "❌ Akses ditolak! Hanya admin yang bisa mengganti password.",
      });
      return;
    }
  ...
  // ===== VIEW LOGS =====
  async handleLogs(sock, sender, messageText, isAdmin) {
    if (!isAdmin) {
      await sock.sendMessage(sender, {
        text: "❌ Akses ditolak! Hanya admin yang bisa melihat logs.",
      });
      return;
    }
  ...
  // ===== RESET PASSWORD =====
  async handleResetPassword(sock, sender, messageText, isAdmin) {
    if (!isAdmin) {
      await sock.sendMessage(sender, {
        text: "❌ Akses ditolak! Hanya admin yang bisa reset password.",
      });
      return;
    }
    ...
    if (parts.length < 1) {
      await sock.sendMessage(sender, {
        text: "❌ Format salah!\n\nGunakan:\n/resetPassword [New Password]\n\nContoh: /resetPassword 070513",
      });
      return;
    }

    const [newPassword] = parts;

    try {
      await resourceService.updateDeletePassword(newPassword, sender);

      await sock.sendMessage(sender, {
        text: `✅ Password berhasil di-reset!\n\n🔑 Password baru: ${newPassword}\n📱 Di-reset oleh: ${sender}`,
      });
    } catch (error) {
      await sock.sendMessage(sender, {
        text: `❌ Gagal reset password: ${error.message}`,
      });
    }
  }

  // Generate report (existing method)
