...
const { formatDate, formatBirthdate, formatDaysRemaining } = require('../utils/formatter');

class BirthdateCommand {
  // ===== SET BIRTH =====
  async handleSetBirth(sock, sender, message) {
    const parts = message.substring(10).trim().split('|').map(p => p.trim());
  ...
  // ===== EDIT BIRTH (TANPA ADMIN CHECK) =====
  async handleEditBirth(sock, sender, message) {
    const parts = message.substring(11).trim().split('|').map(p => p.trim());
  ...
  // ===== DELETE BIRTH (TANPA ADMIN CHECK) =====
  async handleDeleteBirth(sock, sender, name) {
    if (!name) {
      await sock.sendMessage(sender, {
